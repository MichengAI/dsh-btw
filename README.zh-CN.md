...
- 支持普通段落、列表和代码块文字；一次引用最多 8000 个字符，超限会提示，不会截断。
- 引用在划词时固定，正文继续生成不会改变它；其他背景使用发送时主会话已完成的内容。
- 不占用或清空主输入框草稿；跨消息选区、输入框和工具输出不提供该菜单。
- 横条还提供「添加到对话」，将引用追加到已有草稿，不自动发送。右键保留浏览器原有菜单。
...
开发与构建基线为 `0.1.5-rc.2`。`npm run test:compat` 用该基线编译，再在五个隔离宿主版本中分别运行测试集，模型使用测试适配器。DOM 组件测试不替代真实浏览器验收。
