...
- 已安装 DeepSeek Harness。开发与构建基线为 `0.1.5-rc.1`。
- 宿主 peer 依赖精确声明为 `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1`。
- 执行 `npm run test:compat`，用最新版依赖编译，再在四个隔离版本中分别运行 43 项测试。覆盖编译后的宿主与客户端入口、真实子代理 fork、上下文继承、工具禁用、取消及父会话继续运行；矩阵的模型适配器使用测试实现；另在 `0.1.5-rc.1` 上通过真实 Edge 与 DeepSeek 模型端到端验收，覆盖旁问、附件拒绝、复制、折叠、取消和父会话继续。旧三版尚未执行浏览器端到端。
