...
开发与构建基线为 `0.1.5-rc.1`。`npm run test:compat` 用该基线编译，再在四个隔离宿主版本中分别运行 43 项测试，模型使用测试适配器。

另在 `0.1.5-rc.1` 上完成真实 Edge 与 DeepSeek 模型的手工端到端验收，旁问、附件拒绝、复制、折叠/展开、取消和父会话继续均通过。这不属于 `npm run test:compat`；旧三版尚未执行浏览器端到端验收。
