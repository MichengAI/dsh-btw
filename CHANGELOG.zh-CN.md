# 更新日志

## [未发布]

- 记录 `0.1.5-rc.1` 上真实 Edge 与 DeepSeek 模型的手工端到端验收：旁问、附件拒绝、复制、折叠/展开、取消及父会话继续均通过；此验收独立于 `npm run test:compat`，旧三版尚未执行浏览器端到端。
- 将锁文件下载地址统一至 npm 官方源，保持依赖版本及 integrity 校验值不变。

- 适配 DSH `0.1.5-rc.1` 的统一附件接口，拒绝图片和普通文件，保留旧版图片计数协议。
- 宿主 peer 依赖精确声明为 `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1`，开发依赖及锁文件统一升级至 `0.1.5-rc.1`。
- 移除旧宿主不存在的独立 ui-chat 注入要求，保留会话插槽接入。
- 新增 `npm run test:compat`：最新版编译产物在四版隔离宿主各通过 43 项测试，包含真实子代理 fork、上下文隔离、工具禁用、取消与父会话继续运行；模型使用测试适配器。
- 宿主测试默认使用项目依赖，不再因本机缺少 DSH 而跳过；补充附件拒绝回归并验证新旧宿主工具保护、RPC 和卸载清理。
- README 图片改用固定提交的绝对地址，不增加 npm 包内图片文件。
- GitHub Release 列表查询失败时停止发布操作，补充创建、更新与异常分支测试。
- 补充真实 React 组件测试，覆盖 Markdown 安全渲染、复制及重试、折叠展开和关闭。

## [0.1.4] - 2026-09-07

- 从 npm 发布包中排除仅供仓库展示的截图和品牌图片，在保留 GitHub README 图片的同时缩小安装包体积。
- 删除独立 mock 预览及仅验证该 mock 的浏览器测试，避免再将其当作真实 DSH 插件页面。

## [0.1.3] - 2026-09-06

- 修复 GitHub Actions 安装包路径，确保 npm 将 tar 包识别为本地文件，并在正式发布前通过 npm publish dry run 验证。
- 包含原定 0.1.2 的 OIDC 授权检查及授权失败、凭据处理测试；0.1.2 发布在上传安装包前失败。

## [0.1.2] - 2026-09-06

- 为 GitHub Actions 的不发布验证增加 npm 可信发布检查，通过 OIDC 身份交换短期发布凭据，验证授权但不上传安装包。
- 补充发布流程的授权失败和凭据处理测试。插件功能保持不变。

## [0.1.1] - 2026-09-06

- 限制每个会话最多保留 20 个旁问气泡；满额先关闭最旧的已完成气泡，成功后接收新问题，失败则保留答案和当前输入并显示中英文提示。
- 关闭清理失败时显示本地化重试提示，宿主错误详情仅记录日志。
- 修复资源清理失败覆盖已完成答案的问题，保留关闭重试能力。
- 为启动取消和资源清理增加有界等待，继续接管迟到的子代理；卸载异常不再阻塞插件退出，未释放子代理仍受工具保护。
- 按会话保留最近的旁问气泡，避免其他会话的提问挤掉答案；淘汰时清理失败会保留气泡供重试。
- 将中英文 README 的社区说明统一为引用块格式。

## [0.1.0] - 2026-09-06

- 新增 `/btw` 一次性旁问，继承主会话已完成回合，只回答、不执行工具。
- 在输入框上方展示独立 Markdown 答案气泡，支持复制、折叠、关闭和取消清理。
- 适配 DSH 浅色、深色主题及中英文界面，隐藏内部传输命令。
- 不占用侧边栏，不记录输入历史，不绑定上下键。
- 提供安装说明、实际界面截图及自动测试、打包和双语 Release 工作流。
- 提供完整中英文 README，统一采用 Apache-2.0 许可证。
