...
const options = { repository: 'owner/repo', tag: 'v0.1.5', notes: './artifacts/notes.md' }
...
test('成功查询后仅创建不存在的版本，并校验标签', () => {
  ...
  assert.deepEqual(calls[1], ['gh', ['release', 'create', options.tag, '--repo', options.repository, '--verify-tag', '--title', options.tag, '--notes-file', options.notes]])
})

test('已有版本仅更新说明，不上传安装包', () => {
  const { run, calls } = fixture([{ status: 0, stdout: 'v0.1.4\nv0.1.5\n' }, { status: 0 }])
  publishRelease(options, run)
  assert.deepEqual(calls.slice(1).map(call => call[1][1]), ['edit'])
})

test('预发布版本带标记；更新失败时停止', () => {
